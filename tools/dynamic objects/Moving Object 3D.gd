class_name MovingObject3D extends DynamicPlatform3D
...
func _ready():
	...
	if $"Path3D".get_child_count() == 0:
		for i in number_of_objects:
			...
			path_follow_agents_tween.resize(path_follow_agents.size())
...
func _create_path_tween(object_to_tween : PathFollow3D, halt_at_end: bool) -> void:
	...
	var i = 0
	for _path_follow in path_follow_agents:
		if _path_follow == object_to_tween:
	...
	elif loop and object_to_tween.progress_ratio == 1.0:
...
func _stop_tween(object_to_stop : PathFollow3D):
	var i := 0
	for _path_follow in path_follow_agents:
		if _path_follow == object_to_stop:
...
func linear_movement(delta : float) -> void:
	for _path_follow in path_follow_agents:
		var new_ratio = _path_follow.progress_ratio
		new_ratio += (speed_of_platform / path_length) * delta
		if new_ratio > 1:
			new_ratio -= 1.0
		path_follow.progress_ratio = new_ratio

func add_another_path_follow(_path_follow : PathFollow3D, index : int) -> void:
	var new_ratio = ((1.0/number_of_objects)) * (index)
	if new_ratio > 1.0:
		new_ratio -= int(new_ratio)
	_path_follow.progress_ratio = new_ratio
